...
foc_result foc_core::run_control_from_isr(uint32_t timestamp_us)
{
    ...
    float electrical_angle = foc_math::normalize_angle(
        mechanical_angle * (float)core_context.config.rotor_direction *
    ...
    if(target.mode == foc_control_mode::VOLTAGE)
    {
...
foc_result foc_core::init(const foc_config &config)
{
    ...
    if(!target_topic.init() || !telemetry_topic.init() ||
        !fault_request_topic.init() || !snapshot_topic.init())
    {
        return foc_result::TOPIC_ERROR;
    }
